...
export class ShopService {
  ...
  getProducts(useCache = true): Observable<Pagination<Product[]>> {
    ...
    if(this.productCache.size > 0 && useCache) {
      if(this.productCache.has(Object.values(this.shopParams).join('_'))) {
        this.pagination = this.productCache.get(Object.values(this.shopParams).join('_'));
    ...
    return this.http.get<Pagination<Product[]>>(this.baseUrl + 'products', { params }).pipe(
      map(response => {
        this.productCache.set(Object.values(this.shopParams).join('_'), response)
